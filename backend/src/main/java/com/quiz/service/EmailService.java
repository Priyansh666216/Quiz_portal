...
public class EmailService {
    ...
    public void sendQuizResultEmail(
        ...
        try {
            ...
            String html = """
                ...
                <head>
                  ...
                  <style>
                    ...
                    .card{background:#ffffff;border-radius:12px;max-width:560px;margin:0 auto;
                    ...
                    .grade-badge{display:inline-block;background:%s;color:#fff;font-size:36px;
                                 font-weight:bold;border-radius:50%%;width:72px;height:72px;
                                 line-height:72px;text-align:center;margin-bottom:20px;}
                    .row{display:flex;justify-content:space-between;
                         border-bottom:1px solid #f0f0f0;padding:10px 0;font-size:14px;}
                    .row:last-child{border-bottom:none;}
                    .label{color:#666;}
                    .value{font-weight:bold;color:#1a1a1a;}
                    .badge{display:inline-block;background:#f0c040;color:#1a1a1a;font-size:11px;
                           font-weight:bold;border-radius:4px;padding:2px 8px;margin-left:8px;}
                    .footer{background:#f9f9f9;padding:16px 32px;font-size:12px;
                            color:#999;text-align:center;}
                    .pdf-note{background:#f0f9ff;border:1px solid #bae6fd;border-radius:8px;
                              padding:12px 16px;margin-top:16px;font-size:13px;color:#0369a1;}
                  </style>
                </head>
                <body>
                  <div class="card">
                    ...
                    <div class="body">
                      ...
                      <div class="row"><span class="label">Student</span><span class="value">%s</span></div>
                      <div class="row"><span class="label">Email</span><span class="value">%s</span></div>
                      <div class="row"><span class="label">Quiz</span>
                        <span class="value">%s<span class="badge">%s</span></span></div>
                      <div class="row"><span class="label">Category</span><span class="value">%s</span></div>
                      <div class="row"><span class="label">Score</span><span class="value">%d / %d</span></div>
                      <div class="row"><span class="label">Correct</span><span class="value">%d / %d</span></div>
                      <div class="row"><span class="label">Percentage</span><span class="value">%.1f%%</span></div>
                      <div class="row"><span class="label">Grade</span>
                        <span class="value" style="color:%s;font-size:18px;">%s</span></div>
                      <div class="pdf-note">
                        📎 Full answer sheet with all submitted answers is attached as a PDF.
                      </div>
                    </div>
                    <div class="footer">QuizMaster Pro &nbsp;·&nbsp; Automated Notification</div>
                  </div>
                </body>
                </html>
                """.formatted(
            ...
            String subject = "📊 Quiz Submitted — " + studentUsername
                    + " | " + quizTitle + " | Grade: " + grade;

            String json = """
                {
                  "sender": {"name": "QuizMaster Pro", "email": "priyanshushukla73700@gmail.com"},
                  "to": [{"email": "%s"}],
                  "subject": "%s",
                  "htmlContent": "%s",
                  "attachment": [
            ...
            if (responseCode == 201) {
                log.info("✅ Result email sent via Brevo for {} — {}", studentUsername, quizTitle);
            } else {
                String response = new String(conn.getErrorStream().readAllBytes());
                log.error("❌ Brevo API error {}: {}", responseCode, response);
            }
        ...
        if (questions == null || questions.isEmpty()) {
            ...
            for (Question q : questions) {
                ...
                Font statusFont = new Font(Font.FontFamily.HELVETICA, 10, Font.BOLD,
                        submitted == null ? textGray
                        : isCorrect ? new BaseColor(22, 163, 74)
                ...
                if (isCoding) {
                    ...
                    PdfPCell codeBoxCell = new PdfPCell(new Phrase(codeText, codeFontWhite));
                    ...
                    for (int i = 0; i < 4; i++) {
                        ...
                        String prefix;
                        if (isCorrectOpt) {
                            optBg = new BaseColor(220, 252, 231); optFont = correctF; prefix = "✓ ";
                        } else if (isSubmittedOpt) {
                            optBg = new BaseColor(254, 226, 226); optFont = wrongF;   prefix = "✗ ";
                        } else {
                            optBg = lightBg; optFont = bodyFont; prefix = "   ";
                        }
